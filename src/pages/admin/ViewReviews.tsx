...
function formatSubmittedAtIst(submittedAt: string): string {
...
export const ViewReviews: React.FC = () => {
  ...
  const availableMonths = useMemo(() => {
    if (!reviews) return [];
    const months = Array.from(new Set(reviews.map((r) => r.review_month)))
      .sort()
      .reverse();
    return months;
  }, [reviews]);
  ...
  const getQuestionText = (review: Review): string => {
    return (
      ...
      `Question #${review.question?.id || "Unknown"}`
    );
  };

  const groupReviewsByQuestion = (
    reviewsToGroup: Review[],
  ): Record<string, Review[]> => {
    const grouped: Record<string, Review[]> = {};
    reviewsToGroup.forEach((review) => {
      const key = review.question?.id?.toString() || "unknown";
      if (!grouped[key]) {
        grouped[key] = [];
      }
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            {Object.entries(groupReviewsByQuestion(filteredReviews)).map(
              ([questionId, questionReviews]) => (
                <Card key={questionId}>
                  <h3 className="text-lg font-bold text-gray-900 mb-4">
                    {getQuestionText(questionReviews[0])}
                    <span className="ml-2 text-sm font-normal text-gray-500">
                      ({questionReviews.length} response
                      {questionReviews.length !== 1 ? "s" : ""})
                    </span>
                  </h3>
                      ...
                      >
                        ...
                        <div className="text-xs text-gray-500">
                          <div className="font-medium text-gray-700 mb-1">
                            {formatSubmittedAtIst(review.submitted_at)}
                          </div>
                          <div>
                            {formatSubmittedAtIst(review.submitted_at)} • Month:{" "}
                            {review.review_month}
                          </div>
                        </div>
                      </div>
